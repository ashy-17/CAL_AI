...
import os, json, base64, requests, datetime
...
# Uses Firebase Admin SDK to send push notifications via FCM HTTP v1 API.
# 1. Go to Firebase Console → Project Settings → Service Accounts
# 2. Click "Generate new private key" → download the JSON file
# 3. Set FIREBASE_SERVICE_ACCOUNT_PATH to its path, OR paste the JSON as
#    FIREBASE_SERVICE_ACCOUNT_JSON in your .env
# 4. Copy your FIREBASE_VAPID_KEY from Firebase Console →
#    Project Settings → Cloud Messaging → Web Push certificates → Key pair
# ─────────────────────────────────────────────────────────────────────────────
_firebase_app = None

def _init_firebase():
    """Lazy-initialise Firebase Admin SDK. Returns True on success."""
    ...
    try:
        ...
        # Option B: JSON string stored directly in env var
        ...
        if sa_path and os.path.exists(sa_path):
            ...
            print('[FCM] ❌ No service account set. '
        ...
        print(f'[FCM] ❌ Init failed: {_e}')
        return False

# Expose the VAPID/web-push key used by the JS SDK (Firebase "applicationServerKey")
# Found in Firebase Console → Project Settings → Cloud Messaging → Web Push certificates
FIREBASE_VAPID_KEY = os.environ.get('FIREBASE_VAPID_KEY', '').strip()

def _send_fcm(fcm_token: str, data_dict: dict):
    """
    Send a push notification via FCM HTTP v1 API.
    Returns (ok: bool, error: str|None).
    'fcm_token' is the registration token obtained from getToken() in the browser.
    """
    if not _init_firebase():
        return False, 'Firebase Admin SDK hindi naka-initialize.'
    try:
        from firebase_admin import messaging
        msg = messaging.Message(
            token=fcm_token,
            notification=messaging.Notification(
                title=data_dict.get('title', '🥗 Cal AI'),
                body=data_dict.get('body', 'Time to log your meal!'),
            ),
            webpush=messaging.WebpushConfig(
                notification=messaging.WebpushNotification(
                    title=data_dict.get('title', '🥗 Cal AI'),
                    body=data_dict.get('body', 'Time to log your meal!'),
                    icon=data_dict.get('icon', '/static/icons/icon-192.png'),
                    badge='/static/icons/icon-72.png',
                    vibrate_duration_millis=300,
                    actions=[
                        messaging.WebpushNotificationAction(action='log',  title='✏️ Log Food'),
                        messaging.WebpushNotificationAction(action='scan', title='📷 Scan Food'),
                    ],
                    custom_data={
                        'url': data_dict.get('url', '/dashboard'),
                        'requireInteraction': 'false',
                    },
                ),
                fcm_options=messaging.WebpushFCMOptions(
                    link=data_dict.get('url', '/dashboard'),
                ),
            ),
        )
        messaging.send(msg)
        ...
        err = str(e)
        print(f'[FCM] send failed token={fcm_token[:8]}... error={err}')
        # FCM returns UNREGISTERED / INVALID_ARGUMENT when the token is stale
        if 'UNREGISTERED' in err or 'registration-token-not-registered' in err:
            return False, 'SUBSCRIPTION_EXPIRED'
        return False, err

# ── Jinja context: Firebase config ────────────────────────────────────────────
@app.context_processor
def inject_firebase_config():
    """Expose Firebase project config to every template (used in base.html)."""
    return dict(
...
def firebase_messaging_sw():
    """Serve Firebase messaging SW from root scope (required by FCM)."""
    from flask import Response
    sw_path = os.path.join(os.path.dirname(__file__), 'static', 'js', 'firebase-messaging-sw.js')
    with open(sw_path) as f:
        content = f.read()
    # Inject Firebase config values as self.* variables so the SW can read them
    config_js = (
        f"self.FIREBASE_API_KEY             = '{os.environ.get('FIREBASE_API_KEY', '')}';\n"
        f"self.FIREBASE_AUTH_DOMAIN         = '{os.environ.get('FIREBASE_AUTH_DOMAIN', '')}';\n"
        f"self.FIREBASE_PROJECT_ID          = '{os.environ.get('FIREBASE_PROJECT_ID', '')}';\n"
        f"self.FIREBASE_STORAGE_BUCKET      = '{os.environ.get('FIREBASE_STORAGE_BUCKET', '')}';\n"
        f"self.FIREBASE_MESSAGING_SENDER_ID = '{os.environ.get('FIREBASE_MESSAGING_SENDER_ID', '')}';\n"
        f"self.FIREBASE_APP_ID              = '{os.environ.get('FIREBASE_APP_ID', '')}';\n"
    )
    full = config_js + '\n' + content
    response = Response(full, mimetype='application/javascript')
    response.headers['Service-Worker-Allowed'] = '/'
    response.headers['Cache-Control'] = 'no-cache'
    return response
...
def dashboard():
    ...
    ''', (uid,)).fetchall()
    weekly = [{'log_date': str(r['log_date']), 'cal': float(r['cal'] or 0)} for r in weekly_raw]
    ...
    # Load today's saved water glasses if table exists
    water_today = 0
    try:
        row = db.execute('SELECT glasses FROM user_water WHERE user_id = %s AND date = CURDATE()', (uid,)).fetchone()
        if row:
            water_today = int(row['glasses'])
    except Exception:
    ...
    return render_template('dashboard.html',
        ...
        cache_login=pending, water_today=water_today, user_times=user_times)
...
def analytics():
    ...
    by_cat_raw = db.execute('''
        SELECT f.category, ROUND(SUM(f.calories * fl.quantity / 100),1) as total_cal
        FROM food_logs fl JOIN foods f ON fl.food_id = f.id
        WHERE fl.user_id = %s AND fl.logged_at >= DATE_SUB(CURDATE(), INTERVAL 29 DAY)
        GROUP BY f.category ORDER BY total_cal DESC
    ''', (uid,)).fetchall()
    # Explicitly convert to plain dicts so tojson serializes correctly
    by_cat = [{'category': r['category'] or 'Other', 'total_cal': float(r['total_cal'] or 0)} for r in by_cat_raw]
    ...
    for i in range(29, -1, -1):
        ...
        if day_str in row_map:
            r = row_map[day_str]
            trend.append({'d': day_str, 'cal': float(r.get('cal') or 0),
                          'prot': float(r.get('prot') or 0),
                          'carb': float(r.get('carb') or 0),
...
def profile():
    ...
    if request.method == 'POST':
        ...
        return redirect(url_for('profile'))
    goal_calories = calculate_goal(user)
    user_times = _get_user_times(session['user_id'])
    return render_template('profile.html', user=user, goal_calories=goal_calories, user_times=user_times)
...
@login_required
def push_subscribe():
    """Store the FCM registration token sent by the JS Firebase SDK."""
    db  = get_db()
    uid = session['user_id']
...
def send_test_push():
    ...
    db  = get_db()
    row = db.execute('SELECT subscription_json FROM push_subscriptions WHERE user_id = %s',
                     (session['user_id'],)).fetchone()
    if not row:
        return jsonify({'error': 'Walang push subscription. I-enable muna ang notifications sa Profile page.'}), 404
    stored = json.loads(row['subscription_json'])
    fcm_token = stored.get('fcm_token') if isinstance(stored, dict) else stored
    push_data = {
        'title': '🔔 Cal AI Test',
    ...
    ok, err = _send_fcm(fcm_token, push_data)
    if ok:
        return jsonify({'ok': True})
    if err == 'SUBSCRIPTION_EXPIRED':
        db.execute('DELETE FROM push_subscriptions WHERE user_id = %s', (session['user_id'],))
        db.commit()
        return jsonify({'error': 'FCM token expired. I-toggle ulit ang notifications.'}), 410
    return jsonify({'error': err or 'Push failed'}), 500

@app.route('/api/push/debug')
@login_required
def push_debug():
    """Debug endpoint — returns FCM and subscription status."""
    db  = get_db()
    row = db.execute('SELECT subscription_json, created_at FROM push_subscriptions WHERE user_id = %s',
                     (session['user_id'],)).fetchone()
    ...
    try:
        ...
        sdk_installed = False
    return jsonify({
        'firebase_sdk_installed': sdk_installed,
        'firebase_initialized':   firebase_ok,
        'vapid_key_set':          bool(FIREBASE_VAPID_KEY),
        'subscribed':             row is not None,
        'subscription_created':   str(row['created_at']) if row else None,
        'status': '✅ Ready' if (firebase_ok and row) else '❌ Not ready',
    })
...
@login_required
def api_water():
    """GET: return today's water glasses for the current user.
       POST: set today's water glasses (body: {count: N}).
       The table `user_water` is created automatically if missing.
    """
    db = get_db()
    uid = session['user_id']
    # Ensure table exists (simple migration-on-demand)
    try:
        db.execute('''CREATE TABLE IF NOT EXISTS user_water (
        ...
        print(f'[DB] Could not ensure user_water table: {e}')

    today = datetime.datetime.now().date()
    if request.method == 'GET':
        row = db.execute('SELECT glasses FROM user_water WHERE user_id = %s AND date = %s', (uid, today)).fetchone()
        return jsonify({'count': int(row['glasses']) if row else 0})
    ...
    try:
        db.execute('''INSERT INTO user_water (user_id,date,glasses) VALUES (%s,%s,%s)
                      ON DUPLICATE KEY UPDATE glasses=%s, updated_at=CURRENT_TIMESTAMP''',
                   (uid, today, count, count))
        db.commit()
        return jsonify({'ok': True, 'count': count})
    except Exception as e:
        print(f'[DB] Failed to save water for user {uid}: {e}')
        return jsonify({'error': str(e)}), 500
...
def get_vapid_key():
    """Return the Firebase Web Push certificate key (used by Firebase JS SDK getToken)."""
    return jsonify({'key': FIREBASE_VAPID_KEY})
...
def _push_to_user(user_id, sub_json, meal_id):
    """Send a push notification to one user for the given meal_id via FCM."""
    ...
    if ok:
        ...
        try:
            with app.app_context():
                ...
                db.commit()
            print(f'[Push] Removed expired FCM token for user {user_id}')
        except Exception as ex:
            print(f'[Push] Could not remove expired token for user {user_id}: {ex}')
    else:
...
def _fire_reminder(meal_id, current_hhmm):
    ...
    """
    try:
        print(f'[Push] _fire_reminder running for {meal_id} at {current_hhmm}')
        with app.app_context():
            db = get_db()
            subs = db.execute(
                'SELECT ps.user_id, ps.subscription_json FROM push_subscriptions ps'
            ).fetchall()
            print(f'[Push] found {len(subs)} subscriptions to evaluate')
            for row in subs:
                uid = row['user_id']
...
def push_reminders():
    ...
    times = _get_user_times(session['user_id'])
    schedule = [
        {'id': meal_id, 'label': REMINDER_META[meal_id]['label'], 'time': times[meal_id]}
        for meal_id in REMINDER_META
    ]
    return jsonify({'schedule': schedule, 'timezone': os.environ.get('TZ', 'Asia/Manila')})

# ─── Auto-start scheduler ────────────────────────────────────────────────────
    # Works with: python app.py  |  flask run  |  gunicorn app:app
    # WERKZEUG_RUN_MAIN guard prevents double-start from Flask's reloader.
    # When Flask debug reloads, the actual server runs in the child process where
    # WERKZEUG_RUN_MAIN == 'true', so we should start the scheduler there.
if os.environ.get('WERKZEUG_RUN_MAIN') == 'true' or not app.debug:
    start_scheduler()
    # Warn if email is not configured (forgot-password will silently fail)
